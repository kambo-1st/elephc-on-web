//! Purpose:
//! Emits wasm32-web array_map lowering for five-source mixed array assignments.
//!
//! Called from:
//! - `crate::codegen::wasm::expr::array_map_multi_emitters`.
//!
//! Key details:
//! - Uses mixed argument cells and PHP null-fill for shorter indexed sources.
//! - Supports compact integer and value-cell indexed sources without native codegen changes.

use super::*;

pub(in crate::codegen::wasm::expr) fn emit_array_map_five_compact_int_locals_assign(
    name: &str,
    first: &str,
    second: &str,
    third: &str,
    ...
    if [first, second, third, fourth, fifth].iter().any(|source| {
        module.local_kind(source) != Some(LocalKind::Array) || module.array_layout(source) != ArrayLayout::CompactInt
    }) {
        return Err(CompileError::new(
            source_span,
            "wasm32-web five-array array_map() currently requires compact integer arrays",
        ));
    }
    let (Some(first_len), Some(second_len), Some(third_len), Some(fourth_len), Some(fifth_len)) = (
        ...
        return emit_array_map_five_runtime_compact_int_locals_assign(
            name, first, second, third, fourth, fifth, callback, module,
        );
    };
    if first_len != second_len || first_len != third_len || first_len != fourth_len || first_len != fifth_len {
        return Err(CompileError::new(
            source_span,
            "wasm32-web five-array array_map() currently rejects unequal lengths until PHP null-fill semantics are implemented",
        ));
    }
...
fn emit_array_map_five_runtime_compact_int_locals_assign(
    name: &str,
    first: &str,
    second: &str,
    third: &str,
    ...
    let index = module.next_label("array_map_five_index");
    let done_label = module.next_label("array_map_five_done");
    let loop_label = module.next_label("array_map_five_loop");
    module.declare_i32_local(index.trim_start_matches('$').to_string());
    module.clear_array_length(name);
    module.set_array_layout(name, ArrayLayout::CompactInt);
    module.set_array_value_cell_kinds(name, None);
    module.set_array_nested_value_metadata(name, None);
    for source in [second, third, fourth, fifth] {
...
pub(in crate::codegen::wasm::expr) fn emit_array_map_five_value_string_locals_assign(
    name: &str,
    first: &str,
    second: &str,
    third: &str,
    ...
    if [first, second, third, fourth, fifth].iter().any(|source| {
        ...
        return Err(CompileError::new(
            source_span,
            "wasm32-web five-array array_map() currently requires string value-cell arrays",
        ));
    }
    let (Some(first_len), Some(second_len), Some(third_len), Some(fourth_len), Some(fifth_len)) = (
        module.array_length(first),
        ...
        return emit_array_map_five_runtime_value_string_locals_assign(
            name, first, second, third, fourth, fifth, callback, module,
        );
    };
    if first_len != second_len || first_len != third_len || first_len != fourth_len || first_len != fifth_len {
...
fn emit_array_map_five_runtime_value_string_locals_assign(
    name: &str,
    first: &str,
    second: &str,
    third: &str,
    ...
    let index = module.next_label("array_map_five_string_index");
    let result_cell = module.next_label("array_map_five_string_result_cell");
    let done_label = module.next_label("array_map_five_string_done");
    let loop_label = module.next_label("array_map_five_string_loop");
...
pub(in crate::codegen::wasm::expr) fn emit_array_map_five_mixed_locals_assign(
    name: &str,
    first: &str,
    second: &str,
    third: &str,
    ...
    for source in [first, second, third, fourth, fifth] {
            ...
            || !matches!(module.array_layout(source), ArrayLayout::CompactInt | ArrayLayout::Value | ArrayLayout::Assoc)
        {
            return Err(CompileError::new(
                source_span,
                "wasm32-web mixed five-array array_map() currently requires compact/value/associative arrays",
            ));
        }
    }
    let result_kind = match module.function_return_kind(callback) {
        ...
        _ => {
            return Err(CompileError::new(
                source_span,
                "wasm32-web mixed five-array array_map() currently requires int, string, bool, or float callback results",
            ));
        }
    };
    let result_len = module.next_label("array_map_mixed_five_len");
    let index = module.next_label("array_map_mixed_five_index");
    let result_cell = module.next_label("array_map_mixed_five_result_cell");
    let first_arg = module.next_label("array_map_mixed_five_first_arg");
    let second_arg = module.next_label("array_map_mixed_five_second_arg");
    let third_arg = module.next_label("array_map_mixed_five_third_arg");
    let fourth_arg = module.next_label("array_map_mixed_five_fourth_arg");
    let fifth_arg = module.next_label("array_map_mixed_five_fifth_arg");
    let source_cell = module.next_label("array_map_mixed_five_source_cell");
    for local in [
        &result_len,
        &index,
        &result_cell,
    ...
    if let (Some(first_len), Some(second_len), Some(third_len), Some(fourth_len), Some(fifth_len)) = (
        module.array_length(first),
        module.array_length(second),
        module.array_length(third),
        module.array_length(fourth),
        ...
        let len = first_len.max(second_len).max(third_len).max(fourth_len).max(fifth_len);
    ...
    module.body().line("i32.const 0");
    module.body().line(&format!("local.set {}", index));
    let loop_label = module.next_label("array_map_mixed_five_loop");
    let done_label = module.next_label("array_map_mixed_five_done");
    module.body().open(&format!("block {}", done_label));
    module.body().open(&format!("loop {}", loop_label));
    module.body().line(&format!("local.get {}", index));
    module.body().line(&format!("local.get {}", result_len));
    ...
    match result_kind {
        ...
        _ => unreachable!("mixed five-array array_map() result kind was checked"),
    }
    module.body().line(&format!("local.get {}", index));
    module.body().line("i32.const 1");
    module.body().line("i32.add");
    ...
    module.body().close("end");
    module.body().close("end");
    for arg in [&first_arg, &second_arg, &third_arg, &fourth_arg, &fifth_arg] {
