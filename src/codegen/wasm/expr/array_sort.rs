...
fn emit_sort_callable_descriptor(
    ...
    if callable_return_expr_targets(expr, module).is_some() {
        ...
        if kind != ValueKind::Callable {
            return Err(CompileError::new(
                expr.span,
                "wasm32-web sort callable descriptor expected a callable value",
            ));
        }
        return Ok(());
    }
...
pub(super) fn emit_uasort_call(
    ...
    let Some(callback) = evaluated_static_callback_function_name(&args[1], module)? else {
        ...
        if emit_uasort_dynamic_callable_descriptor_call(source, &args[1], module)? {
            return Ok(ValueKind::Bool);
        }
        return Err(CompileError::new(
            args[1].span,
...
fn emit_uasort_dynamic_callable_descriptor_call(
    source: &str,
    callback_expr: &Expr,
    module: &mut WasmModule,
) -> Result<bool, CompileError> {
    ...
    for callback in &callbacks {
        if !module.has_function(callback) {
            return Err(CompileError::new(
                callback_expr.span,
                "wasm32-web dynamic uasort() callable descriptor can only target declared user functions",
            ));
        }
        let shape = usort_callback_shape(callback, callback_expr.span, module)?;
        if callback_shape.is_some_and(|existing| existing != shape) {
            return Err(CompileError::new(
                callback_expr.span,
                "wasm32-web dynamic uasort() callable descriptor currently requires callbacks with matching comparator signatures",
            ));
        }
        callback_shape = Some(shape);
    }
    ...
    if !uasort_local_supports_shape(source, callback_shape, module) {
        return Ok(false);
    }

    let callback_id = module.next_label("uasort_callable_id");
    let matched = module.next_label("uasort_callable_matched");
    for local in [&callback_id, &matched] {
        module.declare_i32_local(local.trim_start_matches('$').to_string());
    }
    emit_sort_callable_descriptor(callback_expr, module)?;
    ...
    for callback in callbacks {
        ...
        module.body().line("i32.eq");
        module.body().open("if");
        emit_uasort_supported_local(source, &callback, callback_shape, module);
        module.body().line("i32.const 1");
        module.body().line(&format!("local.set {}", matched));
        module.body().close("end");
    }
