...
fn collect_possible_callable_param_targets_in_stmt(
    ...
    if let StmtKind::Assign { name, value } | StmtKind::TypedAssign { name, value, .. } = &stmt.kind {
        if let Some(targets) = callable_targets_for_param_metadata(value, local_callable_targets) {
    ...
    collect_array_param_calls_in_stmt(stmt, &mut |name, args| {
        ...
        for (index, kind) in param_kinds.iter().enumerate() {
            ...
            let value = arg_for_param(args, param_names, index)
                .or(default)
                .and_then(|arg| callable_targets_for_param_metadata(arg, local_callable_targets));
...
fn callable_targets_for_param_metadata(
    ...
    match &expr.kind {
        ...
        ExprKind::Ternary {
            ...
            let mut targets = callable_targets_for_param_metadata(then_expr, local_callable_targets)?;
            for target in callable_targets_for_param_metadata(else_expr, local_callable_targets)? {
