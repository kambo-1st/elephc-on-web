//! Purpose:
//! Emits wasm32-web array_map lowering for three-source array assignments.
//!
//! Called from:
...
pub(in crate::codegen::wasm::expr) fn emit_array_map_three_compact_int_locals_assign(
    name: &str,
    first: &str,
    second: &str,
    third: &str,
    ...
    if [first, second, third].iter().any(|source| {
        module.local_kind(source) != Some(LocalKind::Array) || module.array_layout(source) != ArrayLayout::CompactInt
    }) {
        return Err(CompileError::new(
            source_span,
            "wasm32-web three-array array_map() currently requires compact integer arrays",
        ));
    }
    let (Some(first_len), Some(second_len), Some(third_len)) = (
        module.array_length(first),
        module.array_length(second),
        module.array_length(third),
    ) else {
        return emit_array_map_three_runtime_compact_int_locals_assign(
            name, first, second, third, callback, module,
        );
    };
    if first_len != second_len || first_len != third_len {
        return Err(CompileError::new(
            source_span,
            "wasm32-web three-array array_map() currently rejects unequal lengths until PHP null-fill semantics are implemented",
        ));
    }
...
pub(in crate::codegen::wasm::expr) fn emit_array_map_three_runtime_compact_int_locals_assign(
    name: &str,
    first: &str,
    second: &str,
    third: &str,
    ...
    let index = module.next_label("array_map_triple_index");
    let done_label = module.next_label("array_map_triple_done");
    let loop_label = module.next_label("array_map_triple_loop");
...
pub(in crate::codegen::wasm::expr) fn emit_array_map_three_value_string_locals_assign(
    name: &str,
    first: &str,
    second: &str,
    third: &str,
    ...
    if [first, second, third].iter().any(|source| {
        ...
        return Err(CompileError::new(
            source_span,
            "wasm32-web three-array array_map() currently requires string value-cell arrays",
        ));
    }
    let (Some(first_len), Some(second_len), Some(third_len)) = (
        module.array_length(first),
        ...
        return emit_array_map_three_runtime_value_string_locals_assign(
            name, first, second, third, callback, module,
        );
    };
    if first_len != second_len || first_len != third_len {
...
pub(in crate::codegen::wasm::expr) fn emit_array_map_three_mixed_locals_assign(
    name: &str,
    first: &str,
    second: &str,
    third: &str,
    ...
    for source in [first, second, third] {
            ...
            || !matches!(module.array_layout(source), ArrayLayout::CompactInt | ArrayLayout::Value | ArrayLayout::Assoc)
        {
            return Err(CompileError::new(
                source_span,
                "wasm32-web mixed three-array array_map() currently requires compact/value/associative arrays",
            ));
        }
    }
    let result_kind = match module.function_return_kind(callback) {
        ...
        _ => {
            return Err(CompileError::new(
                source_span,
                "wasm32-web mixed three-array array_map() currently requires int, string, bool, or float callback results",
            ));
        }
    };
    let result_len = module.next_label("array_map_mixed_triple_len");
    let index = module.next_label("array_map_mixed_triple_index");
    let result_cell = module.next_label("array_map_mixed_triple_result_cell");
    let first_arg = module.next_label("array_map_mixed_triple_first_arg");
    let second_arg = module.next_label("array_map_mixed_triple_second_arg");
    let third_arg = module.next_label("array_map_mixed_triple_third_arg");
    let source_cell = module.next_label("array_map_mixed_triple_source_cell");
    for local in [
        &result_len,
        &index,
        &result_cell,
    ...
    if let (Some(first_len), Some(second_len), Some(third_len)) = (
        module.array_length(first),
        module.array_length(second),
        module.array_length(third),
    ) {
        let len = first_len.max(second_len).max(third_len);
        ...
        module.body().line(&format!("local.get ${}_len", third));
        module.body().line("i32.gt_u");
        module.body().open("if (result i32)");
        module.body().line(&format!("local.get {}", result_len));
        module.body().close("else");
    ...
    module.body().line("i32.const 0");
    module.body().line(&format!("local.set {}", index));
    let loop_label = module.next_label("array_map_mixed_triple_loop");
    let done_label = module.next_label("array_map_mixed_triple_done");
    module.body().open(&format!("block {}", done_label));
    module.body().open(&format!("loop {}", loop_label));
    module.body().line(&format!("local.get {}", index));
    module.body().line(&format!("local.get {}", result_len));
    ...
    match result_kind {
        ...
        _ => unreachable!("mixed three-array array_map() result kind was checked"),
    }
    module.body().line(&format!("local.get {}", index));
    module.body().line("i32.const 1");
    module.body().line("i32.add");
    ...
    module.body().close("end");
    module.body().close("end");
    for arg in [&first_arg, &second_arg, &third_arg] {
...
pub(in crate::codegen::wasm::expr) fn emit_array_map_three_runtime_value_string_locals_assign(
    name: &str,
    first: &str,
    second: &str,
    third: &str,
    ...
    let index = module.next_label("array_map_triple_string_index");
    let cell = module.next_label("array_map_triple_string_cell");
    let done_label = module.next_label("array_map_triple_string_done");
    let loop_label = module.next_label("array_map_triple_string_loop");
    module.declare_i32_local(index.trim_start_matches('$').to_string());
