...
pub(in crate::codegen::wasm) fn emit_array_filter_assign(
    ...
    match &args[0].kind {
        ...
        {
            let temp = materialize_dynamic_static_method_array_filter_source(
                &args[0], receiver, method, module,
            )?;
            emit_array_filter_staged_assign(name, &temp, args[0].span, &callback, shape, module)
        }
...
fn materialize_dynamic_static_method_array_filter_source(
    source: &Expr,
    receiver: &StaticReceiver,
    method: &Expr,
    module: &mut WasmModule,
    ...
    let temp = module
        .next_label("array_filter_dynamic_static_method_source")
        .trim_start_matches('$')
        .to_string();
    module.declare_array_local(temp.clone());
    match emit_expr(source, module)? {
