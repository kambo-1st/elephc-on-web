...
pub(super) fn emit_indexed_array_pad_assign(
    ...
    match &args[0].kind {
        ...
        {
            let temp = materialize_dynamic_static_method_array_pad_source(
                &args[0],
                receiver,
                method,
                module,
...
fn materialize_dynamic_static_method_array_pad_source(
    source: &Expr,
    receiver: &StaticReceiver,
    method: &Expr,
    module: &mut WasmModule,
    ...
    let metadata = dynamic_static_method_call_array_return_metadata(receiver, method, module).ok_or_else(|| {
        CompileError::new(
            source.span,
            "wasm32-web array_pad() requires dynamic static method array return metadata",
        )
    })?;
    let temp = module
        .next_label("array_pad_dynamic_static_method_source")
        .trim_start_matches('$')
        .to_string();
    module.declare_array_local(temp.clone());
    match emit_expr(source, module)? {
        ...
        _ => {
            return Err(CompileError::new(
                source.span,
                "wasm32-web array_pad() expected an array-returning dynamic static method value",
            ));
        }
    }
    module.set_array_layout(&temp, metadata.layout);
